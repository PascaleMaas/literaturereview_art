...
osf_retrieve_node(raw_data_node_id) %>%
  osf_ls_files(pattern = "2025-03-26-10-14-9IE3DZIS.md") %>%
...
} else if (any(basename(md_file) == "2025-03-26-10-14-9IE3DZIS.md")) {
...
# Initialize a dataframe to store parsed data (without type and color)
...
# Updated metadata pattern without Type and Color:
...
metadata_pattern <- paste0(metadata_pattern, "\\*\\*Date:\\*\\*\\s*\\((.*?)\\)\\s*\\|\\s*")  
...
for (line in md_lines) {
  ...
  if (!is.na(metadata_match[2])) {
    ...
    # Extract annotation text: prefer the first alternative (group 3), fallback to group 4 if needed
    ...
    parsed_df <- rbind(
      ...
      data.frame(
        ...
        date       = trimws(metadata_match[7]),
        title      = trimws(metadata_match[8]),
        stringsAsFactors = FALSE
      )
    )
  } else {
